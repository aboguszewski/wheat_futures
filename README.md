## Wheat Future Price Forecasting
This repository showcases exploration and feature engineering using real world weather, macroeconomic, international trade and futures market datasets.  
The datasets are ready for modelling!

### Sources
- Weather and Futures datasets - [*Guillem SD*](https://www.kaggle.com/guillemservera), Kaggle
- Consumer Product Index data - [*Federal Reserve Bank of St. Louis*](https://fred.stlouisfed.org/series/CPIAUCSL)
- Global Wheat Production data - [*United States Department of Agriculture Foreign Agricultural Service*](https://apps.fas.usda.gov/psdonline/app/index.html#/app/advQuery)

### Additional Information
- Very brief description of KC HRW Wheat can be found [*here*](https://www.marketswiki.com/wiki/Hard_Red_Winter_Wheat)
- Units used for global production data can be found [*here*](https://ipad.fas.usda.gov/countrysummary/Default.aspx?id=US&crop=Wheat)